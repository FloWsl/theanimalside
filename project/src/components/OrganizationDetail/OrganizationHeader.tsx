...
const OrganizationHeader: React.FC<OrganizationHeaderProps> = ({ organization }) => {
  ...
  return (
    <div className="relative overflow-hidden min-h-screen flex items-center">
      {/* Immersive Adventure Background */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: `url(${organization.heroImage})` }}
      >
        {/* Enhanced signature adventure gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#2C392C]/85 via-[#2C392C]/70 to-[#2C392C]/40" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#D2691E]/5 to-transparent" />
      </div>
      
      {/* Enhanced floating adventure elements - mobile optimized */}
      <div className="absolute top-16 right-8 w-24 h-24 lg:w-40 lg:h-40 bg-gradient-to-br from-[#D2691E]/25 to-[#8B4513]/20 rounded-full blur-3xl animate-pulse-golden" />
      <div className="absolute bottom-28 left-6 w-20 h-20 lg:w-32 lg:h-32 bg-gradient-to-br from-[#87A96B]/30 to-[#2C392C]/25 rounded-full blur-2xl animate-float" />
      <div className="hidden lg:block absolute top-1/3 right-1/4 w-24 h-24 bg-gradient-to-br from-[#8B4513]/15 to-[#D2691E]/10 rounded-full blur-xl animate-float" style={{animationDelay: '3s'}} />
      
      {/* Adventure Content - mobile responsive */}
      <div className="relative z-10 container-nature-wide">
        <div className="pb-12 lg:pb-20 text-white">
          <div className="max-w-5xl space-y-8 lg:space-y-12 pt-4 lg:pt-8">
            
            {/* Enhanced Adventure Location + Trust - mobile responsive */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:justify-between">
              <div className="flex items-center gap-2 bg-gradient-to-r from-[#8B4513]/95 to-[#D2691E]/85 backdrop-blur-md px-4 lg:px-5 py-2 lg:py-3 rounded-full border border-white/30 shadow-2xl">
              ...
              {reviewCount > 0 && (
                <div className="flex items-center gap-2 lg:gap-3 bg-white/20 backdrop-blur-md px-4 lg:px-5 py-2 lg:py-3 rounded-full border border-white/40 shadow-2xl">
              ...
              <div className="sm:hidden text-white/80 text-body">
              ...
              <div className="flex flex-col lg:flex-row items-center lg:items-start gap-6 lg:gap-10 text-center lg:text-left">
                <div className="flex-shrink-0 relative">
                  {/* Enhanced logo glow effect - mobile optimized */}
                  <div className="absolute -inset-2 lg:-inset-3 bg-gradient-to-br from-sunset/40 to-rich-earth/35 rounded-3xl blur-xl lg:blur-2xl" />
                  ...
                  {organization.verified && (
                    <div className="absolute -bottom-1 -right-1 flex items-center gap-1 bg-green-500/90 backdrop-blur-sm px-2 py-1 rounded-full shadow-xl border border-green-400/50">
                ...
                <div className="space-y-4 lg:space-y-6">
                  <div>
                    <h1 className="text-hero text-white drop-shadow-2xl">
                      {organization.name}
                    </h1>
                  </div>
                  
                  {/* Enhanced elegant info chips - mobile responsive */}
                  <div className="flex flex-wrap justify-center lg:justify-start gap-2 lg:gap-4">
                    <div className="bg-white/15 backdrop-blur-md px-3 lg:px-4 py-1.5 lg:py-2 rounded-full border border-white/30 shadow-xl">
            ...
            {/* Smooth transition description - mobile responsive */}
            <div className="text-center max-w-3xl mx-auto px-4">
              <p className="text-body-large text-white/90 font-light drop-shadow-lg">
                Work hands-on with rescued {organization.animalTypes.map(a => a.animalType.toLowerCase()).slice(0, -1).join(', ')} and {organization.animalTypes[organization.animalTypes.length - 1]?.animalType.toLowerCase()} in Costa Rica's rainforest. Help with daily care, rehabilitation, and release programs while living among the wildlife you're protecting.
              </p>
            </div>
          </div>
        </div>
      </div>
      {/* Enhanced adventure transition */}
      <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-[#F8F3E9] via-[#F8F3E9]/80 to-transparent" />
    </div>
  );
};
