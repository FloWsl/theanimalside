...
const EssentialInfoSidebar: React.FC<EssentialInfoSidebarProps> = ({ 
  ...
  const mainProgram = selectedProgram; // Use the selected program for essential info
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    ...
    requirements: isDesktop ? true : false, // Always expanded on desktop
  ...
  React.useEffect(() => {
    if (isDesktop) {
      // On desktop, expand all sections for better UX
      setExpandedSections({
        cost: true,
        duration: true,
        requirements: true,
  ...
  const toggleSection = (section: string) => {
    // Only allow toggling on mobile - desktop sections stay expanded
    if (!isDesktop) {
  ...
  const ResponsiveCard: React.FC<{ 
    ...
    // On desktop, remove toggle functionality entirely
    ...
    return (
      <div className={`bg-white rounded-2xl shadow-nature border border-beige/60 overflow-hidden ${
        ...
        {shouldShowToggle && (
          <button
            onClick={onToggle}
            className="w-full p-4 sm:p-6 flex items-center justify-between bg-gradient-to-r from-white to-beige/20 border-b border-beige/30 hover:bg-beige/10 transition-colors duration-200 min-h-[44px] touch-manipulation"
        ...
        {expanded && (
          <div 
            ...
            className={isDesktop ? "p-4" : (expandable ? "p-4 sm:p-6" : "p-4 sm:p-6")}
          >
            {children}
          </div>
        )}
  ...
  return (
    <div className={`space-y-4 sm:space-y-6 ${isDesktop ? 'lg:space-y-3' : ''} ${className}`}>
      {/* Quick Facts Header - Always visible for orientation */}
      <ResponsiveCard>
        <h2 className={`text-xl sm:text-2xl text-forest mb-2 sm:mb-4 font-bold ${
          isDesktop ? 'lg:text-lg lg:mb-2' : ''
        }`}>Essential Information</h2>
        <p className={`text-sm sm:text-base text-forest/70 ${
      ...
      <ResponsiveCard>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-forest">Cost</h3>
        </div>
        
        <div className="space-y-4">
          {/* Mobile-optimized cost display */}
          <div className="text-center bg-gradient-to-r from-rich-earth/5 to-sunset/5 rounded-xl p-4">
            ...
            <div className="text-sm sm:text-base text-forest/70 mt-1">
          ...
          <div className="space-y-3">
            <h4 className="font-medium text-forest text-sm sm:text-base">What's Included:</h4>
            <div className="grid grid-cols-1 gap-2">
              {mainProgram.cost.includes.slice(0, 4).map((item, index) => (
                <div key={index} className="text-sm sm:text-base text-forest/80 flex items-center gap-3 p-2 bg-sage-green/5 rounded-lg">
      ...
      <ResponsiveCard>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-forest">Duration</h3>
        </div>
        
        <div className="space-y-4">
          {/* Mobile-optimized duration display */}
          <div className="text-center bg-gradient-to-r from-sunset/5 to-golden-hour/5 rounded-xl p-4">
          ...
          <div className="space-y-3">
            <div className="bg-sunset/5 rounded-lg p-3">
      ...
      >
        ...
        {organization.skillRequirements.required.length > 0 && (
          <div className="space-y-3">
            <h4 className="font-medium text-forest text-sm sm:text-base flex items-center gap-2">
              <Clock className="w-4 h-4 text-sunset" />
              Essential Requirements
            </h4>
            <div className="space-y-2">
              {organization.skillRequirements.required.slice(0, 4).map((requirement, index) => (
                <div key={index} className="text-sm sm:text-base text-forest/80 flex items-center gap-3 p-3 bg-sunset/5 rounded-lg">
                  <div className="w-2 h-2 bg-sunset rounded-full flex-shrink-0" />
                  <span className="leading-relaxed">{requirement}</span>
                </div>
              ))}
              {organization.skillRequirements.required.length > 4 && (
                <div className="text-sm text-sunset font-medium text-center p-2 bg-sunset/10 rounded-lg">
      ...
      >
        <div className="flex flex-wrap gap-3">
          {organization.languages.map((language, index) => (
            <span 
              key={index}
              className="px-4 py-2 bg-sage-green/10 text-sage-green rounded-full text-sm sm:text-base font-medium flex items-center gap-2"
            >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="text-center bg-gradient-to-r from-rich-earth/5 to-warm-beige/20 rounded-xl p-4">
            <div className="text-lg sm:text-xl font-bold text-forest capitalize">
              {organization.accommodation.provided ? 
                organization.accommodation.type.replace('_', ' ') : 
                'Not Provided'
              }
            </div>
            <div className="text-sm sm:text-base text-forest/70 mt-2 leading-relaxed">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                {organization.accommodation.amenities.slice(0, 6).map((amenity, index) => (
                  <div key={index} className="text-sm sm:text-base text-forest/80 flex items-center gap-3 p-2 bg-rich-earth/5 rounded-lg">
        ...
        <div className="space-y-4">
          ...
          <div className="text-center bg-gradient-to-r from-sunset/5 to-golden-hour/5 rounded-xl p-4">
            ...
            {organization.meals.provided && (
              <div className="text-sm sm:text-base text-forest/70 mt-2 capitalize">
                {organization.meals.type.replace('_', ' ')}
              </div>
            )}
            {organization.meals.provided && (
              <div className="text-sm sm:text-base text-forest/70 mt-1 leading-relaxed">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {organization.meals.dietaryOptions.map((option, index) => (
                  <span 
                    key={index}
                    className="px-3 py-2 bg-sunset/10 text-sunset rounded-full text-sm sm:text-base font-medium"
                  >
                    {option}
                  </span>
                ))}
      ...
      <ResponsiveCard>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-forest">Location & Access</h3>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-sage-green/5 rounded-lg p-3">
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                <span className="text-forest/70 text-sm sm:text-base">Nearest Airport:</span>
                <span className="font-semibold text-forest text-sm sm:text-base text-right leading-relaxed">
          ...
          <div className="bg-gradient-to-r from-sage-green/5 to-rich-earth/5 rounded-xl p-4">
            <h4 className="font-medium text-forest text-sm sm:text-base mb-3">Services & Connectivity</h4>
            <div className="grid grid-cols-3 gap-4">
              <div className="flex flex-col items-center gap-2 p-3 bg-white/70 rounded-lg">
                ...
                <span className="text-xs sm:text-sm text-forest/80 text-center font-medium">
                  Airport Pickup
                </span>
              ...
              <div className="flex flex-col items-center gap-2 p-3 bg-white/70 rounded-lg">
                ...
                <span className="text-xs sm:text-sm text-forest/80 text-center font-medium capitalize">
