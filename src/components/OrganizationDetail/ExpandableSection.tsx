      ...
      <Card className={`${config.bgColor} ${config.borderColor} border-2 hover:shadow-md transition-all duration-300 overflow-hidden`}>
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className={`text-xl font-display font-semibold ${config.textColor} group-hover:text-rich-earth transition-colors`}>
                    {title}
                  </h3>
                </div>
                ...
                {!isExpanded && preview && (
                  <p className="text-body-small text-deep-forest/70 line-clamp-2">
                    {preview}
                  </p>
                )}
              </div>
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              <CardContent className="pt-0 pb-6 px-4 lg:px-6">
                {/* Border separator */}
                ...
                {!forceExpanded && !isDesktop && (
                  <div className="mt-4 pt-4 border-t border-deep-forest/5">
                    <button
                      onClick={toggleExpanded}
                      className="w-full flex items-center justify-center gap-2 text-body-small text-deep-forest/60 hover:text-rich-earth transition-colors py-2 touch-manipulation min-h-[44px]"
                    >
